...
export const didRegisterAndImport = async (agent: Agent) => {
  await registerBcovrinDid(agent, "stringstringstringstringstringst");
};
...
const registerBcovrinDid = async (
  ...
  if (response.data && response.data.did) {
    await importDid(agent, "bcovrin:testnet", response.data.did, seed);
  } else {
    throw new Error("Invalid response from Bcovrin registration");
  }
};
